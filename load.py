...
# in docker, if /tmp/fcclicense is a mounted volume, we won't have access rights
# to remove the entire folder, so we'll remove the individual files instead 
try:
    os.unlink(workingdir + "fcc-license.zip")
    os.unlink(workingdir + "fcc_lic_vw.csv")
    print "Removed cached files"
except:
    print "Error removing cached files, continuing..."
